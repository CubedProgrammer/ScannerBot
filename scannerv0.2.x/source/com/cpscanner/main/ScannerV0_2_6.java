...
public class ScannerV0_2_6
{
	...
	public static final String[]getCmdArgs(String s)
	{
		...
		{
			if(cs[i]==ScannerV0_2_6.BRACKET&&!escaped)
			{
				open=!open;
			}
			else if(cs[i]==ScannerV0_2_6.DELIMITER&&!open)
			{
				argsal.add(builder.toString());
				builder.delete(0,builder.length());
			}
			else if(!escaped&&cs[i]==ScannerV0_2_6.ESCAPE)
			{
				escaped=true;
			}
			else
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public ScannerV0_2_6()throws LoginException
	{
		this.jda=new JDABuilder(AccountType.BOT).setToken(ScannerV0_2_6.TOKEN).build();
		try
		{
			this.jda.awaitReady();
		}
	...
	public String parseChangeChannel(Guild guild,User author,long channel,String...args)
	{
		...
		if(args.length>=2)
		{
			...
			if(!ScannerV0_2_6.isValidNumber(sguild))
			{
	...
	public String parseEntityInfo(Guild guild,User user,long channel,String...args)
	{
		...
		if(args.length>=1)
		{
			...
			else if(args[0].length()>4&&args[0].charAt(0)=='<'&&args[0].charAt(args[0].length()-1)=='>'&&args[0].charAt(1)=='@')
			{
				...
				Date join=new Date((id>>22)+ScannerV0_2_6.EPOCH);
	...
	public String parseSum(Guild guild,User user,long channel,String...args)
	{
		...
		{
			n=n.add(ScannerV0_2_6.strToNum(args[i]));
		}
		return n.toString();
	}
	/**
	...
	public String parseProduct(Guild guild,User user,long channel,String...args)
	{
		...
		for(int i=0;i<args.length;i++)
		{
			n=n.multiply(ScannerV0_2_6.strToNum(args[i]));
		}
		return n.toString();
	}
	/**
	...
	public String parseRemoveKillMessage(Guild guild,User author,long channel,String...args)
	{
		...
		if(args.length==1)
		{
			...
			try
			{
				PrintStream out=new PrintStream(new FileOutputStream(guild.getId()+"/killwords.txt"));
				for(int i=0;i<this.kills.length;i++)
				{
					out.println(this.kills[i]);
				}
				out.close();
			}
			catch(Exception e)
	...
	public String parseListKillMessages(Guild guild,User author,long channel,String...args)
	{
	...
	public String parseGetEpoch(Guild guild,User author,long channel,String...args)
	{
		return Long.toString(ScannerV0_2_6.EPOCH);
	}
	/**
	...
	public void run1()
	{
		...
		try
		{
			...
			while(s!=null)
			{
				out.println(this.consoleCommandParser.parse(null,null,0,ScannerV0_2_6.getCmdArgs(s)));
		...
		while(this.running)
		{
			...
			if(this.channel!=null&&msg!=null)
			{
				if(!printed)
				{
					...
					args=ScannerV0_2_6.getCmdArgs(msg.getContentRaw());
					if(args.length>0)
					{
						if(args[0].equals("<@!"+Long.toString(ScannerV0_2_6.ID)+">"))
						{
